...
print(f"Descriptive Stats: {df.describe()}")

print(f"Mean Temperature: \n{df["Temperature (c)"].mean()}")
print(f"Median Temperature: \n{df["Temperature (c)"].median()}")
print(f"Mode Temperature: \n{df["Temperature (c)"].mode()}")

print(f"Variance Temperature: \n{df["Temperature (c)"].var()}")
print(f"Standard Deviation Temperature: \n{df["Temperature (c)"].std()}")
...
for column in numerical_cols:
   fig = px.box(df_no_outliers, y=column,
                title=f"Box Plot of {column}", labels={column: column})
   fig.show()

# Plot histograms
for column in numerical_cols:
   fig = px.histogram(df_no_outliers, x=column, nbins=30,
                      title=f"Histogram of {column}", labels={column: column})
   fig.show()

# Skewness
skewness_values = {}
for col in numerical_cols:
   gp = 3 * (df_no_outliers[col].mean() -
             df_no_outliers[col].median()) / df_no_outliers[col].std()
...
for ax, col in zip(axes.flatten(), numerical_cols):
    ...
    ax.axvline(df_no_outliers[col].median(),
               color='red', linestyle='dashed', label='Median')
    ax.axvline(df_no_outliers[col].mean(),
               color='blue', linestyle='dashed', label='Mean')
    ax.legend()
...
# by default, sample variance (ddof=1)
var_sal = df_no_outliers['Salinity (ppt)'].var()
# sample standard deviation
std_sal = df_no_outliers['Salinity (ppt)'].std()
...
print(
    f"Salinity: mean={mean_sal:.2f}, variance={var_sal:.2f}, std={std_sal:.2f}")
print(
    f"Temperature: mean={mean_temp:.2f}, variance={var_temp:.2f}, std={std_temp:.2f}")
print(
    f"Dissolved O2: mean={mean_odo:.2f}, variance={var_odo:.2f}, std={std_odo:.2f}")

# Compute z-scores for each observation in Salinity, Temperature, and ODO
df_no_outliers['Salinity_z'] = (
    df_no_outliers['Salinity (ppt)'] - mean_sal) / std_sal
df_no_outliers['Temperature_z'] = (
    df_no_outliers['Temperature (c)'] - mean_temp) / std_temp
...
print(df_no_outliers[['Salinity (ppt)', 'Salinity_z',
      'Temperature (c)', 'Temperature_z', 'ODO mg/L', 'ODO_z']].head())
...
pearson_temp_sal = df_no_outliers['Temperature (c)'].corr(
    df['Salinity (ppt)'], method='pearson')
spearman_temp_sal = df_no_outliers['Temperature (c)'].corr(
    df['Salinity (ppt)'], method='spearman')
...
fig = px.imshow(df_no_outliers[numerical_cols].corr(),
